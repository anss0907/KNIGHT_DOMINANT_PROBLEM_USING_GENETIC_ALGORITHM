...
{ // reseting the board
    ...
    // Filling the boards with knights and attacks
    for (int pop = 0; pop < __POP__; pop++)
    {
        for (int k = 0; k < __MAX__KNIGHTS__; k++)
        {
            int r = chromosome[pop][k] / __BOARD_SIZE__;
            int c = chromosome[pop][k] % __BOARD_SIZE__;
            Boards[pop][r][c] = __KNIGHT__;
            ...
            for (int iter = 0; iter < 8; iter++)
            {
                ...
                if (iter % 2)
                    ...
                    b = -b;
                if (r + a >= 0 && c + b >= 0 && r + a < __BOARD_SIZE__ && c + b < __BOARD_SIZE__ && Boards[pop][r + a][c + b] != __KNIGHT__)
                    Boards[pop][r + a][c + b] = __ATTACK__;
            }
        }
    }
}
